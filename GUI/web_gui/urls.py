...
urlpatterns = [
    ...
    re_path(r'^(?P<job_title>\w+)/(?P<actions>\w+)/edit/(?P<subaction>\w+)$', views.event_manager_edit, name = 'manage_edit'),
    ...
    re_path(r'^(?P<job_title>\w+)/(?P<actions>\w+)/delete/(?P<id_num>\d+)$', views.deleting, name = 'jobs_delete_item'),
